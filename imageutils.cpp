...
ImageUtils::ImageUtils(const QString& filePath) {
  ...
  if (fileInfo->exists() && fileInfo->isFile()) {
    ...
    throw std::exception(
        (QString::number(ERROR_WRONG_FILE_TYPE)).toStdString().c_str());
  }
}
...
QString ImageUtils::cannyCheck(const QString& imagePath) const {
  ...
  QString tempImagePath =
      File::getTempDir().append(".").append(getImageType(imagePath));
...
QString ImageUtils::denoisingImage() const {
  ...
  QString tempPath = File::getTempDir().append(".").append(
      getImageType(this->fileInfo->absoluteFilePath()));
